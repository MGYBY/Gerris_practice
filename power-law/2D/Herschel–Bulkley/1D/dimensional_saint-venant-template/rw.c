...
// the same dimensionless parameters as Liu & Mei (1994): alpha = 0.3, beta = 27, similar to manufactured solutions (MMS)
#define normalDepth 0.078846821
#define normalVel 0.85761226
#define soVal 0.06
#define grav 9.81
#define GVal (grav*1.0/pow((1+soVal*soVal),0.50))
#define tauCVal 17.372223
#define nCoeffVal 1.0
#define muN 1.0
#define rhoFluidVal 1250.0

#define FR (normalVel/pow(GVal*normalDepth,0.50))
...
#define distPeriod (2.0*normalDepth/normalVel/soVal)
#define OUTPUTTIME 2.00
#define DOMAINLENGTH (300.0*normalDepth/soVal)

// initial shape parameters
#define xo (3.0*normalDepth/soVal)
#define halfWidth (4.0*normalDepth/soVal/2.0)
#define humpAmp 0.20

#define uThreshold (normalVel/1000.0)
#define dryThreshold (1.0e-5)

#define initRefLenEnd1 (xo-halfWidth*1.50)
#define initRefLenEnd2 (xo+halfWidth*6.0)
#define refTime (2.*halfWidth/normalVel*5.0)
...
#define UNIFORMLEVEL 13

#define WARMUP (0.00)


// h[left] = dirichlet( (t-WARMUP)<=(distPeriod/2.0) && (t-WARMUP)>=0 ? INLETDEPTHBC(disMag, (t-WARMUP), distPeriod, normalDepth) : (normalDepth) );
// u.n[left] = dirichlet( (t-WARMUP)<=(distPeriod/2.0) && (t-WARMUP)>=0 ? FR*pow((GVal*INLETDEPTHBC(disMag, (t-WARMUP), distPeriod, normalDepth)), 0.5) : (normalVel) );
...
event init(i = 0)
{
     foreach ()
     {
          ...
          h[] = (fabs(x-xo)<=halfWidth) ? (normalDepth*(1.+humpAmp*(1.-fabs(x-xo)/halfWidth))) : normalDepth;
...
event friction(i++)
{
     ...
     foreach ()
     {
          ...
          u.x[] = ((u.x[]>uThreshold)&&(h[]>dryThreshold)&&(rhoFluid*G*h[]*so>tauC)) ? ((u.x[]+dt*(G*so-tauC/rhoFluid/h[]))/(1.0+dt*muN*pow((u.x[]*h[]),(nCoeff-1.0))/rhoFluid*pow(((pow((rhoFluid*G*so),2.0)*(nCoeff+1.0)*(2.0*nCoeff+1.0))/((rhoFluid*G*h[]*so-tauC)*(nCoeff*(nCoeff+1.0)*rhoFluid*G*h[]*so+nCoeff*nCoeff*tauC))),nCoeff))) : 0.0;
...
event adapt1 (i++) {
     adapt_wavelet({h, depthGrad}, (double[]){(normalDepth/275.0), 0.00650}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
//      astats s = adapt_wavelet({h, depthGrad}, (double[]){1.0/300.0, 0.00016}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  // astats s = adapt_wavelet({ depthGrad}, (double[]){ 0.00010}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
     ...
     refine(x>=initRefLenEnd1 && x<=initRefLenEnd2 && t<=refTime && level < MAXLEVEL);
}
