...
void flux (const double * state, double * flux, double * eigenvalue, double betaCoeff);
		       ...
		       double dtmax)
{
  ...
  if (right[0]>=dryThreshold && left[0]>=dryThreshold) {
    flux (right, fr, er, betaCoeff);
    flux (left,  fl, el, betaCoeff);
