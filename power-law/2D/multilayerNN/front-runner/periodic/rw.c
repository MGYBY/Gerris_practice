...
#define DOMAINLENGTH (25.0)
#define PLOTRANGEMAX (5.00*normalDepth)

#define distWL 2.0
#define refLen (distWL*3.0)
 ...
 */
event adapt1 (i++) {
  adapt_wavelet({h, depthGrad, uAve}, (double[]){normalDepth/250.0, 0.010, normalVel/250.0}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
...
//   fprintf(stderr, "# refined %d cells, coarsened %d cells\n", s.nf, s.nc);
  refine(x<=refLen && t<=(2.0*(distWL/normalVel)) && level<MAXLEVEL);
}
