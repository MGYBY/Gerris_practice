...
## Initialization  */
event init (i = 0) {
  refine(y>=(Ly-1.1*Lx/pow(2, MAXLEVEL)) && y<=(Ly+1.1*Lx/pow(2, MAXLEVEL)) && level<MAXLEVEL);
  refine(y<=(distWL*2.0) && x<=(distWL*2.25) && level<MAXLEVEL);
  ...
  foreach(){
    ...
    double maxTotalVel = (x<=distWL && x>=(distWL/2.0)) ? pow(totalDepth, 0.50)*(normalMaxVel/normalVel) : pow(normalDepth, 0.50)*(normalMaxVel/normalVel);
    double z = zb[];
...
event hmax(i+=25)
{
     ...
     foreach ()
     {
               if ( h[]>h[1] && h[]>h[-1] && x>maxDepthLocX && h[]>(normalDepth*(1.0+0.250*disAmp))){
                    maxDepth = h[];
                    maxDepthLocX = x;
                    maxDepthVel = u.x[];
               }
