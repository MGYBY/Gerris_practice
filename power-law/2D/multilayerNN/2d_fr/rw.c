...
//   adapt_wavelet({h, depthGrad, uAve, yieldSurf}, (double[]){normalDepth/300.0, 0.007, normalVel/300.0, normalDepth/150.0}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
  ...
  adapt_wavelet({h, uAve, hGradMag, uGradMag}, (double[]){normalDepth/300.0,  normalVel/300.0, 0.0250, 0.0250}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
...
//   fprintf(stderr, "# refined %d cells, coarsened %d cells\n", s.nf, s.nc);
  ...
  refine(x>=(distWL/2.0/1.1) && x<=distWL+refLen && fabs(y)<=(refLen*0.8) && t<=(0.85*(distWL/normalVel)) && level<MAXLEVEL);
...
event output  (t = 0; t <= simTime; t+=outputInterval){
  ...
  sprintf(gerrisName, "outGerris-%g.gfs", t);
  // FILE * fp1 = fopen (s, "w");
