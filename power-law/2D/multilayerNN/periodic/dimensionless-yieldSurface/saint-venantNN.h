...
//     vector up = ul[l] ;
//    //  press -= h*layer[l];
//    // fixme: shear rate formulation for general 2D
...
//    }
  ...
  //     vector um = ul[l];
  //     vector up = ul[l+1];
  //     // double shear = (h>0? (up.x[]-um.x[])/(h*(layer[l]+0.5*layer[l+1]+0.5*layer[l-1])) : HUGE);
  //     double shear = (h>0 ? (up.x[]-um.x[])/(h*(0.5*layer[l]+0.5*layer[l+1])) : HUGE);
  //     nueq[l] = nu_eq(shear);
  //   }
  //   else if (l==(nl-1)) {
  //     // top layer
  ...
  // if (l>0 && l<(nl-1)) {
   ...
   //  press -= h*layer[l];
   // fixme: shear rate formulation for general 2D
    double shear = (h>0? (up.x[]-um.x[])/(h*(layer[l]*0.50+layer[l-1]*0.50)) : HUGE);
    //  nueq[l] =  nu_eq(shear,press);
    nueq[l] =  nu_eq(shear);
  }
  //  else if (l == (nl-1)) {
  ...
  //  }
     ...
     // vector um = ul[l];
     vector up = ul[l];
     double shear = (h>0? (up.x[]*2.0)/(h*layer[l]) : HUGE);
     nueq[l] = nu_eq(shear);
   }
   }
    ...
    */
    ...
    for (l = 1; l < nl - 1; l++) {
      a[l] = - 2.*(nueq[l])*dt/(h*(layer[l-1] + layer[l]));
      c[l] = - 2.*(nueq[l+1])*dt/(h*(layer[l] + layer[l+1]));
    ...
    */
    ...
    a[nl-1] = - 2.*(nueq[l-1])*dt/(h*(layer[nl-2] + layer[nl-1]));
