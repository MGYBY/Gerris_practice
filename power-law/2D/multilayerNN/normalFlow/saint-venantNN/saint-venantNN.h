...
//       vector up1 = ul[l];
//       vector up2 = ul[l+1];
//       double shear = (h>0? ((up1.x[]*layer[l]+up2.x[]*layer[l+1])/(layer[l]+layer[l+1]))/(h*layer[l]) : HUGE);
//       nueq[l] = nu_eq(shear);
//     }
//     else if (l<(nl-1)) {
//       // middle layers
...
//       vector um = ul[l-1];
//       vector up = ul[l];
//       double shear = (h>0? ((um.x[]*layer[l-1]+up.x[]*layer[l])/(layer[l-1]+layer[l]))/(h*layer[l]) : HUGE);
//       nueq[l] = nu_eq(shear);
//     }
//   }
