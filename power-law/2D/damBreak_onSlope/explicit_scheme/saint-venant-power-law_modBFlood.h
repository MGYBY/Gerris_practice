...
// dry threshold is raised since H=1, U=1
double dry = 1e-5;
double betaCoeff = 1.0;
// betaCoeff = (2.0*(1.0+2.0*nCoeff))/(2.0+3.0*nCoeff);
// a dumb parameter
double G = 1.0;
...
// 	hllc (hCNl, hCNr, ul, ur, Delta*cm[]/fm.x[], &fh, &fu, &dtmax);
    kurganovRH (hCNl, hCNr, ul, ur, Delta*cm[]/fm.x[], &fh, &fu, &dtmax);
	fv = (fh > 0. ? vl : vr)*fh*betaCoeff;
