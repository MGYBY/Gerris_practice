...
#include "./saint-venant-implicit-power-law.h"
#include "utils.h"
...
#define PLOTPERIOD 20.0
#define OUTPUTPLOT2 25.00
#define DOMAINLENGTH (initDepth*50.0)
...
// note that for the implicit solver, the unknowns are (h, q)
q.n[right] = neumann(0.);
h[right] = neumann(0.);
...
//      N = 2048;
     ...
     CFLa = 0.25; // CFL number should be sufficiently small
...
// scalar frontXPos[];
// not to be confused with the built-in variables
vector vel[];
...
//                u.x[] = (x>=wavelength/2.0 && x<=wavelength) ? pow(INITDEPTHFUNC(disMag, wavelength, x), 0.5) : 1.0;
               ...
               // note that for the implicit solver, the unknowns are (h, q)
               fr[] = h[]>dry ? q.x[]/(h[]*pow(G*h[], 0.50)) : 0.0;

               vel.x[] = h[]>dry ? q.x[]/h[] : 0.0;
...
event friction(i++)
{
     ...
     foreach ()
     {
          ...
          // note that for the implicit solver, the unknowns are (h, q)
          q.x[] = h[]>dry && (q.x[]/(h[]+1.e-10))>velThreshold  ? (q.x[] + dt*G*so*h[])/(1.0 + dt*mun/rho*pow((1.+2.*n_coeff)/n_coeff,n_coeff)*pow(q.x[], (n_coeff-1.0))/pow(h[], (2.0*n_coeff))) : 0.0;
          ...
          // note that for the implicit solver, the unknowns are (h, q)
          re[] = h[]>dry ? rho*pow((q.x[]/h[]), (2.0-n_coeff))*pow(h[], n_coeff)/mun : 0.0;

          fr[] = h[]>dry ? q.x[]/(h[]*pow((G*h[]),0.50)) : 0.0;
     }
     boundary((scalar *){q.x}); // note that the input should be a list (at least for 1d)
}
...
event hmaxUmax (i+=50)
{
     ...
     foreach ()
     {
           if (h[]>maxDepth){
                maxDepth = h[];
                maxDepthLocX = x;
                maxDepthVel = h[]>dry ? q.x[]/h[] : 0.0;
           }

           if (h[]>dry && q.x[]/(h[]+1.e-10)>maxVel){
                maxVel = q.x[]/h[];
                maxVelLocX = x;
                maxVelDepth = h[];
           }
...
event energyContent(i+=50)
{
     ...
     foreach ()
     {
          te[] = 0.50*rho*h[]*pow(q.x[]/max(dry, h[]), 2.0) + 0.50*rho*G*h[];
          ke[] = 0.50*rho*h[]*pow(q.x[]/max(dry, h[]), 2.0);
          pe[] = 0.50*rho*G*h[];
     }
     stats s1 = statsf (te);
     stats s2 = statsf (ke);
