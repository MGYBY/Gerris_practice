...
void gfs_advection_params_write (GfsAdvectionParams * par, FILE * fp)
{
  ...
  fprintf (fp,
     ...
     par->gradient == gfs_center_va_gradient ?
	   "gfs_center_va_gradient" :
     par->gradient == gfs_center_ospre_gradient ?
	   "gfs_center_ospre_gradient" :
		 par->gradient == gfs_center_vlSharp_gradient ?
	   "gfs_center_vlSharp_gradient" :
     par->gradient == gfs_center_vlSharp04_gradient ?
...
void gfs_advection_params_read (GfsAdvectionParams * par, GtsFile * fp)
{
  ...
  if (gradient) {
    if (!strcmp (gradient, "gfs_center_gradient"))
      ...
      par->gradient = gfs_center_sweby_gradient;
    else if (!strcmp (gradient, "gfs_center_mc_gradient"))
      par->gradient = gfs_center_mc_gradient;
    else if (!strcmp (gradient, "gfs_center_yu_gradient"))
      par->gradient = gfs_center_yu_gradient;
    else if (!strcmp (gradient, "gfs_center_sgvl4_gradient"))
      par->gradient = gfs_center_sgvl4_gradient;
    else if (!strcmp (gradient, "gfs_center_va_gradient"))
      par->gradient = gfs_center_va_gradient;
    else if (!strcmp (gradient, "gfs_center_ospre_gradient"))
      par->gradient = gfs_center_ospre_gradient;
		else if (!strcmp (gradient, "gfs_center_vlSharp_gradient"))
	    par->gradient = gfs_center_vlSharp_gradient;
    else if (!strcmp (gradient, "gfs_center_vlSharp04_gradient"))
	    par->gradient = gfs_center_vlSharp04_gradient;
