...
static gdouble yu_limiter (gdouble r)
{
  return generic_limiter (r, 1.3667);
}

static gdouble mc_limiter (gdouble r)
{
  gdouble v1 = MIN (2.0*r, 0.5*(1.0+r));
  v1 = MIN(v1, 2.0);
  return MAX(v1, 0.0);
}
...
static gdouble va_limiter (gdouble r)
{

  return ((r*(1.0+r))/(r*r+1.0));
}

static gdouble ospre_limiter (gdouble r)
{

  return ((3.0*r*(1.0+r))/(2.0*(r*r+r+1.0)));
}

static gdouble vlSharp_limiter (gdouble r)
{
  gdouble rCorr = MAX(r, 1.0e-5);
  gdouble sharpenX = MIN(rCorr, 1.0/rCorr);
  gdouble sharpenCoeff = 1.0+sharpenX*(1.0-sharpenX)*(1.0-pow(sharpenX, 2.0));
  return (sharpenCoeff*(r+fabs(r))/(1.0+fabs(r)));
}

static gdouble vlSharp04_limiter (gdouble r)
{
  gdouble rCorr = MAX(r, 1.0e-5);
  gdouble sharpenX = MIN(rCorr, 1.0/rCorr);
  gdouble sharpenCoeff = 1.0+sharpenX*(1.0-sharpenX)*(1.0-pow(sharpenX, 0.40));
  return (sharpenCoeff*(r+fabs(r))/(1.0+fabs(r)));
}
...
static gdouble center_yu_gradient (FttCell * cell,
				      FttComponent c,
				      guint v)
{
  return center_limited_gradient (cell, c, v, yu_limiter);
}

static gdouble center_mc_gradient (FttCell * cell,
				      FttComponent c,
				      guint v)
{
...
static gdouble center_va_gradient (FttCell * cell,
				      FttComponent c,
				      guint v)
{
  return center_limited_gradient (cell, c, v, va_limiter);
}

static gdouble center_ospre_gradient (FttCell * cell,
				      FttComponent c,
				      guint v)
{
  return center_limited_gradient (cell, c, v, ospre_limiter);
}

static gdouble center_vlSharp_gradient (FttCell * cell,
				      FttComponent c,
				      guint v)
{
  return center_limited_gradient (cell, c, v, vlSharp_limiter);
}

static gdouble center_vlSharp04_gradient (FttCell * cell,
				      FttComponent c,
				      guint v)
{
  return center_limited_gradient (cell, c, v, vlSharp04_limiter);
}
...
static void fluxPowerLaw (const gdouble * u, gdouble g, gdouble betaVar, gdouble * f)
{
  f[H] = u[H]*u[U];                       /* h*u */
  f[U] = u[H]*(betaVar*u[U]*u[U] + g*u[H]/2.);    /* h*(u*u + g*h/2) */
  f[V] = betaVar*u[H]*u[U]*u[V];                  /* h*u*v */
}
...
static gdouble mmo (gdouble a, gdouble b)
{
  if (a>0 && b>0) {
    if (a>b) return b;
    if (b>a) return a;
...
static void riemann_kurganov_sharp_powerLaw (const GfsRiver * r,
			  const gdouble * uL, const gdouble * uR,
			  gdouble * f, FttCellFace * face)
{
  gdouble cR = sqrt(r->g*uR[H]+SQUARE(uR[U])*(r->betaPowerLaw*r->betaPowerLaw-r->betaPowerLaw));
			  ...
			  gdouble * f, FttCellFace * face)
{
  ...
  gdouble aR = max(uR[U] + cR, uL[U] + cL);
  aR = max(aR, 0.);
  gdouble aL = min(uR[U] - cR, uL[U] - cL);
  aL = min(aL, 0.);
  gdouble qL = uL[H]*uL[U];
  gdouble qR = uR[H]*uR[U];
  gdouble a = max(aR, -aL);
...
static void riemann_kurganov_sharp (const GfsRiver * r,
			  const gdouble * uL, const gdouble * uR,
			  gdouble * f, FttCellFace * face)
{
  gdouble cR = sqrt(r->g*uR[H]);
  ...
  if (a > epsilon) {
    ...
    gdouble qCorr = mmo((uR[H]-wint)/(aR-aL), (wint-uL[H])/(aR-aL));
    f[H] = (aR*qL - aL*qR)/(aR - aL) + aR*aL*((uR[H] - uL[H])/(aR - aL) - qCorr); // (4.5) of [1]
    wint = (aR*qR-aL*qL-((qR*uR[U] + r->g*SQUARE(uR[H])/2.)-(qL*uL[U] + r->g*SQUARE(uL[H])/2.)))/(aR-aL);
    qCorr = mmo((qR-wint)/(aR-aL), (wint-qL)/(aR-aL));
    f[U] = (aR*(qL*uL[U] + r->g*SQUARE(uL[H])/2.) - aL*(qR*uR[U] + r->g*SQUARE(uR[H])/2.))/(aR - aL) + aR*aL*((qR - qL)/(aR - aL)-qCorr);
    // fv = (fh > 0. ? u.y[-1] + dx*gu.y.x[-1] : u.y[] - dx*gu.y.x[])*fh;
    // TODO: V-component flux is now a simple upwind scheme. Follow Kurganov's genuinely 2D method.
    f[V] = (f[H]>0 ? uL[V] : uR[V])*f[H];
    }
...
static void river_run (GfsSimulation * sim)
{
  ...
  r->betaPowerLaw = sim->physical_params.betaCoeff;
  ...
  else if (r->gradient == gfs_center_va_gradient)
    r->gradient = center_va_gradient;
  else if (r->gradient == gfs_center_ospre_gradient)
    r->gradient = center_ospre_gradient;
  else if (r->gradient == gfs_center_vlSharp_gradient)
    r->gradient = center_vlSharp_gradient;
  else if (r->gradient == gfs_center_vlSharp04_gradient)
    r->gradient = center_vlSharp04_gradient;
...
static void river_read (GtsObject ** o, GtsFile * fp)
{
  ...
  if (fp->type == '{') {
	...
	if (river->nlayers > 1)
      ...
      else if (!strcmp (scheme, "kurganovPowerLaw")) {
        if (river->nlayers > 1)
	  gts_file_error (fp, "Kurganov power-law solver can only be used for a single layer");
    else
	  river->scheme = riemann_kurganov_powerLaw;
      }
      else if (!strcmp (scheme, "kurganov")) {
        if (river->nlayers > 1)
	  gts_file_error (fp, "Kurganov power-law solver can only be used for a single layer");
    else
	  river->scheme = riemann_kurganov;
      }
      else if (!strcmp (scheme, "kurganovPowerLawSharp")) {
        if (river->nlayers > 1)
	  ...
	  river->scheme = riemann_kurganov_sharp;
      }
