			  ...
			  gdouble * f, FttCellFace * face)
{
  ...
  gdouble cstar = (cL + cR)/2. + r->betaPowerLaw*(uL[U] - uR[U])/4.;
  gdouble SL = uL[H] == 0. ? uR[U] - 2.*cR : min ((r->betaPowerLaw)*uL[U] - cL, ustar - cstar);
  gdouble SR = uR[H] == 0. ? uL[U] + 2.*cL : max ((r->betaPowerLaw)*uR[U] + cR, ustar + cstar);

  // for debugging
  // printf("beta = %g", r->betaPowerLaw);

  if (0. <= SL)
    fluxPowerLaw (uL, r->g, r->betaPowerLaw, f);
  else if (0. >= SR)
    fluxPowerLaw (uR, r->g, r->betaPowerLaw, f);
  else {
    gdouble fL[3], fR[3];
    fluxPowerLaw (uL, r->g, r->betaPowerLaw, fL);
    fluxPowerLaw (uR, r->g, r->betaPowerLaw, fR);
    f[H] = (SR*fL[H] - SL*fR[H] + SL*SR*(uR[H] - uL[H]))/(SR - SL);
    f[U] = (SR*fL[U] - SL*fR[U] + SL*SR*(uR[H]*uR[U] - uL[H]*uL[U]))/(SR - SL);
    gdouble SM = ((SL*uR[H]*(uR[U] - SR) - SR*uL[H]*(uL[U] - SL))/
		  (uR[H]*(uR[U] - SR) - uL[H]*(uL[U] - SL)));
...
static void river_read (GtsObject ** o, GtsFile * fp)
{
  ...
  if (fp->type == '{') {
	...
	if (river->nlayers > 1)
      ...
      else if (!strcmp (scheme, "hllcPowerLaw")) {
        if (river->nlayers > 1)
	  gts_file_error (fp, "HLLC power-law solver can only be used for a single layer");
    else
	  river->scheme = riemann_hllc_powerLaw;
      }
...
static void river_write (GtsObject * o, FILE * fp)
{
  ...
  fprintf (fp, " {\n"
	   ...
	   river->scheme == riemann_hllc ? "hllc" : (river->scheme == riemann_hllc_powerLaw ? "hllcPowerLaw" :
     (river->scheme == riemann_kurganov_powerLaw ? "kurganovPowerLaw" :(river->scheme == riemann_kurganov ? "kurganov" : "kinetic"))));
  if (river->nu) {
    fputs ("  nu =", fp);
