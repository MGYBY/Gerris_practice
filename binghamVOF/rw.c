...
// 1: fluid, 2: gas
double rho1 = 2.10e3, mu1 = 10.0, rho2 = 1.12, mu2 = 1e-6;
...
*/
  ...
  foreach() {
    u.x[] = velDist(y, tauy, (rho1*GRAV*CHANNELSIN), normalDepth, mu1);
    u.y[] = 0.0;
...
txt files outputs
*/
event txtFileOutput (t = 0.0; t += 0.05; t <= tmax) {
  output_facets (f, fpf); 
  char s[80];
...
gfs files output
*/
event gfsFileOutput (t = 0.0; t += 0.001; t <= tmax) {
  char name[80];
