...
double velDist( double y, double x, double YS, double rhogx, double HN, double mu0, double Lx, double amp){
    ...
    double hDist = HN*(1.0+amp*sin(2.0*pi*x/Lx));
    return y < hDist ? (y<yieldY ? (Up*((2.0*y)/(yieldY)-pow((y)/(yieldY),2.0))) : (Up)) : 0.0;
}
...
event init (i = 0) 
{
  refine((y < normalDepth*2.0) && level < MAXLEVEL);
  CFL = 0.395;
...
*/
event adapt (i++) {
  adapt_wavelet ({f,u}, (double[]){0.002005,uemax,uemax,uemax}, maxlevel = MAXLEVEL, minlevel = MINLEVEL);
}
